...
The weights are Emma-specific importances. Time-related variables ($p$, $b$, $q$) and the time-enjoyment coupling carry high weights because time is the most valuable resource. Cash flow, risk buffer, stability, and the three health floors also carry high weights because their collapse makes the whole life-state fragile.
...
This is the formalization of the conjecture's "time is finite and spent whether or not she chooses where it goes." Even a path with strong per-stage momentum fails if it runs out of horizon before reaching a desirable terminal life-state.
